...
const FORMAL_SPECS: SpecEntry[] = [
  ...
  },
  {
    ...
    status: 'unused',
    statusNote: 'НЕ используется — сознательно отключено. LeadGet API не отдаёт реальный транскрипт переписки, только агрегированные цифры. Раньше слайды сочинялись LLM целиком (выдуманный диалог выдавался за реальный) — это нечестно, поэтому отключили. Можно включить только если появится реальный анонимизированный транскрипт для конкретного кейса.',
  },
];
