...
const concept = `Minimalist chibi mascot (${MASCOT_DESCRIPTION}) sitting cross-legged next to a large blank whiteboard sign propped on simple legs, holding a marker pen pointing at the board. On the board: three simple flat line-art icons in a horizontal row connected by two right-pointing arrows: first icon a small storefront/shop building icon (business owner), second icon a rounded square robot-head icon with two dot eyes and a small antenna (AI bot), third icon a person silhouette with a small checkmark badge next to it (qualified lead). Icons are forest green ${'#1F7A3D'} outline style, evenly spaced, each icon has empty blank space directly below it on the plain board background reserved for a number label (no boxes, no rectangles, no text, no numbers, no digits anywhere on the board or image). Warm cream background, subtle grid texture, flat solid shapes, thick black outline, no gradients, no shadows, no readable text or numbers anywhere.`;
