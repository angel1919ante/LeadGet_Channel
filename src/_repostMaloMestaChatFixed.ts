// Фикс рендера: слайд 4 обрезался (текст результата вылезал за канвас при
// 6 репликах). Renderer теперь сам сжимает плотность при переполнении.
// Удаляем старый альбом, публикуем заново тем же текстом.
