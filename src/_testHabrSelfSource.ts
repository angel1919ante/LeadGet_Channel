// Одноразовый тест: статья для Хабра по новости С Хабра же (row 103,
// Wildberries) — обычно запрещено в generateArticle.ts (самоперепост),
// но это тестовый прогон, не публикация, так что запрет пропускаем вручную.
...
async function main(): Promise<void> {
  ...
  await appendArticles([{ id: `${Date.now()}-habr-test`, platform: 'habr', sourceUrl: row.link, sourceTitle: row.title, content }]);
  console.log(`generated TEST habr article from habr-sourced news, title="${row.title.slice(0, 50)}"`);
}
